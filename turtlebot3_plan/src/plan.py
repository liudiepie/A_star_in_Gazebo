...
# for testing: start: -4 -4 theta: 0 goat: 4 4 clearance: 0.1
...
def generate_trajectory(path, r, L):
    ...
    for index in range(len(path)-1):
        ...
        if action is None:
            continue 
        msg = Twist()
        action[0] = action[0]*1.123
        action[1] = action[1]*1.12
        # compute twist message from given actions (convert to m/s, rad/s)
        msg.linear.x = r*(action[0]+action[1])/20.0 
        msg.angular.z = r*(action[1]-action[0])/(10.0*L)
